...
def apply_risk(proceeds, risk_pct):
    """Return risk-adjusted proceeds: proceeds x (1 - risk_pct)."""
...
def compute_callout_values(
    ...
    # AG10: R&S breakeven exit — mirrors Excel AG10 formula exactly
    # AG11 = desired proceeds (passed in, or fall back to founders_pct * pre_money)
    target = desired if desired is not None else founders_pct * pre_money

    # Excel: IF(AG11 < BQ8, IF(BQ8=0, 0, AG11/BQ8*BO8), IFERROR(linear interp, 0))
    # BQ8 = raise_sell_vals[0], BO8 = exit_vals[0]
    if target < raise_sell_vals[0]:
    ...
    # BM72
    breakeven_multiple = exit_base / post_money if post_money > 0 else 0.0
